...
export async function olLookupIsbn(isbn) {
  ...
  if (!mapped) {
    try {
      const r = await fetch(
        `${OL_URL}?q=isbn:${encodeURIComponent(clean)}&limit=1&fields=key,title,author_name,cover_i,first_publish_year`
      )
      const d = await r.json()
      mapped = (d.docs || []).filter((doc) => doc.title && doc.key).map(mapDoc)
